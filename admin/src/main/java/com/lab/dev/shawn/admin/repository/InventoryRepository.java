...
public interface InventoryRepository extends CrudRepository<Inventory, Long> {
    @Query("""
            SELECT 
            new com.lab.dev.shawn.admin.business.inventory.vo.InventoryResponseVO(a.id,a.concert.name AS concertName,a.session.name sessionName,a.ticketCategory.name ticketCategoryName,a.totalQuantity,a.unallocatedQuantity,a.remainingQuantity,a.status) 
            FROM Inventory a
    ...
    @Query(value = """
            select new com.lab.dev.shawn.admin.base.dto.DropdownOptions(a.id,concat(a.session.name, a.ticketCategory.name,' - 剩余库存：', a.unallocatedQuantity)) 
            from Inventory a 
            where a.deleted=false 
